...
class TestGraphIO:
    ...
    def test_digraph_read_adj_list(self):
        ...
        assert len(g[1].edges_incoming) == 1, 'v1 should have 1 incoming edge (excluding its loop)'
        assert len(g[1].edges_outgoing) == 2, \
            'v1 edges_outgoing should have length 2, since two parallel edges are stored in one ' \
            ' DiEdge object.'
    ...
    def test_multigraph_read_adj_list01(self):
        ...
        assert g[3][2].parallel_edge_count == 2, \
            'there should be 2 parallel edges between (2, 3) [3 edges total]'
        assert g[2][3].parallel_edge_count == 2, \
            'there should be 2 parallel edges between (2, 3) [3 edges total]'
    ...
    def test_multigraph_read_adj_list02(self):
        ...
        assert g.edge_count == g.edge_count_ignoring_parallel_edges(), \
