...
from vertizee.algorithms.search.depth_first_search import \
    BLACK, COLOR, depth_first_search, DFSResults, DepthFirstSearchTree, \
    dfs_labeled_edge_traversal, dfs_postorder_traversal, dfs_preorder_traversal
...
class TestDFSResults:
    ...
    def test_dfs_undirected_cyclic_graph(self):
        ...
        assert len(tree.vertices) == len(dfs.vertices_post_order), 'DFS vertices should match ' \
            'the DFS tree, since only one tree was searched.'
        assert len(tree.edges_in_discovery_order) == len(dfs.edges_in_discovery_order), \
        ...
        assert topological_sort is None, 'Topological sort should be None, since the graph ' \
            'contains cycles.'
    ...
    def test_dfs_undirected_cyclic_graph_with_self_loop(self):
        ...
        assert len(dfs.vertices_pre_order) == len(dfs.vertices_post_order), \
        ...
        assert len(dfs.cross_edges) == 0, 'Graph should have zero cross edges (true for all ' \
            'undirected graphs).'
        assert len(dfs.forward_edges) == 0, 'Graph should have zero forward edges (true for all ' \
            'undirected graphs).'
    ...
    def test_dfs_directed_cyclic_graph(self):
        ...
        assert len(dfs.dfs_forest) == 1, 'DFS search should find 1 DFS tree, since source ' \
            'vertex s was specified.'
        t, *_ = dfs.dfs_forest
        tree: DepthFirstSearchTree = t

        assert len(tree.edges_in_discovery_order) == 4, 'DFS tree rooted at vertex s should ' \
            'have 4 edges.'
        assert len(tree.vertices) == 5, 'DFS tree rooted at vertex s should ' \
            'have 5 vertices.'
        ...
        assert len(dfs.vertices_post_order) == 8, 'All vertices should be accounted for when ' \
            'a source vertex is not specified.'
    ...
    def test_topological_sort(self):
        ...
        assert topo_sorted[1] == 'v' or topo_sorted[1] == 'w', 'Second element of topo sort ' \
            'should be v or w.'
    ...
    def test_dfs_reverse_traversal(self):
        ...
        vertices = list(dfs_preorder_traversal(g, source=0, reverse_graph=True))
        assert vertices == [0, 4, 1, 3, 2] or vertices == [0, 4, 2, 1, 3], \
