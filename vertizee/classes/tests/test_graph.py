...
class TestUndirectedGraphs:
    ...
    def test_edge(self):
        ...
        v1 = g.add_vertex('1')
        e1 = g.add_edge(v0, v1)
        e_loop = g.add_edge(v0)  # Should be able to specify self-loop by providing 1 vertex.
    ...
    def test_graph_initialization_and_parallel_edges(self):
        ...
        assert g.edge_count == 115, \
            f'Graph should have 115 edges, but had {g.edge_count} edges.'
    ...
    def test_iterators(self):
        ...
        assert total_weight == 4968.1, \
            'Iterating over v0 incident edges and summing weights should total 4968.1.'
        assert edge_count == 113, \
            'Iterating over v0 incident edges should include 113 edges.'
        ...
        assert total_degree == (119 + 107 + 3 + 1 + 0), \
    ...
    def test_simple_graph_functionality(self):
        ...
        vs1 = g['s1']
        vs3 = g['s3']
        assert g.current_state_is_simple_graph(), \
        ...
        g.add_edge(vs0, vs0)
        assert not g.current_state_is_simple_graph(), 'Graph should not be a simple graph after ' \
            ' adding a loop.'
        g.convert_to_simple_graph()
        assert g.current_state_is_simple_graph(), \
            'Graph should be a simple graph with no loops and no parallel edges.'
        assert g.edge_count == 4, \
            'Graph should have four edges after conversion to simple graph.'
        ...
        assert g.edge_count == 4, \
            'Graph should have 4 edges after conversion to simple graph.'
        assert len(g.edges) == 4, 'Graph should have 4 edges after conversion to simple graph.'
