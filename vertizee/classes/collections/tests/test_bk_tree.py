...
class TestBKTreeHammingDistance:
    ...
    def test_insert_search(self):
        ...
        assert len(bf_results) == len(results), (
            "The brute-force and BK tree result lengths " "should be the same."
        )
        for node in results:
            assert (
                int(node.key_label),
                node.key_value,
            ) in bf_results, f"BK Tree result node {node} should be in the brute-force result set."
    ...
    def _brute_force_search(
        self, keys: List[Tuple[int, int]], search_key: int, radius: int, search_key_label: str
    ) -> Set[Tuple[int, int]]:
