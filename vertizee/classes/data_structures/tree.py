...
from typing import Generic, List, Iterator, overload, Set
...
class Tree(Generic[V, E]):
    ...
    def __getitem__(self, keys):
        ...
        if data.edges:
            edge_label = edge_module.create_edge_label(
                data.edges[0].vertex1.label, data.edges[0].vertex2.label, graph.is_directed())
            edge = graph._edges[edge_label]
    ...
    def __iter__(self) -> Iterator[V]:
        """Iterates over the vertices of the tree in discovery order."""
    ...
    def add_edge(self, edge: E) -> None:
        ...
        if edge.vertex1 not in self._vertex_set and edge.vertex2 not in self._vertex_set:
            raise exception.Unfeasible(f"neither of the edge endpoints {edge} were found in the "
                "search tree; exactly one of the endpoints must already be in the tree")
        if edge.vertex1 in self._vertex_set and edge.vertex2 in self._vertex_set:
            raise exception.Unfeasible(f"both of the edge endpoints {edge} were found in the "
                "search tree, which would create a cycle; trees are acyclic")
    ...
    def merge(self, other: Tree) -> None:
        ...
        self._edges_in_discovery_order += other._edges_in_discovery_order
        self._vertices_in_discovery_order += other._vertices_in_discovery_order
    ...
    def vertices_in_discovery_order(self) -> ListView[V]:
        ...
        in the tree in order of discovery."""
